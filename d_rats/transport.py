...
class Transporter(object):
        ...
        while ddt2.ENCODED_HEADER in self.inbuf and \
            ...
            try:
                if f.unpack(block):
                    ...
                    print "Transport : Found a broken block (S:%i E:%i len(buf):%i" % (\
                        s, e, len(self.inbuf))
    ...
    def send_frame(self, frame):
        if not self.enabled:
            print "Transport : Refusing to queue block for dead transport"
            return
        ...
        for b in self.outq._queue[:]:
            if b.session == id:
                ...
                try:
                    ...
                    print "Transport : Block disappeared while flushing?"
        self.outq.unlock()
